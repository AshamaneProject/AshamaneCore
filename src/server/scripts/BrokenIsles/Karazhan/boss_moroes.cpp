...
class boss_moroes_new : public CreatureScript
{
    public:
        boss_moroes_new() : CreatureScript("boss_moroes_new")
        {}

        struct boss_moroes_new_AI : public BossAI
        {
            explicit boss_moroes_new_AI(Creature* creature) : BossAI(creature, DATA_MOROES)
            {
                _IsPurged = false;
        ...
        {
            return new boss_moroes_new_AI(creature);
        }
};
