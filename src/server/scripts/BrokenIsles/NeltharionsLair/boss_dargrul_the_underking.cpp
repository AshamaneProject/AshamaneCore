...
class boss_dargrul : public CreatureScript
{
public:
    boss_dargrul() : CreatureScript("boss_dargrul") { }
            ...
            if (Unit* caster = GetCaster())
            {
                ...
                if (!l_TargetList.empty())
                    for (Unit* l_Target : l_TargetList)
                        if (l_Target->ToCreature() && l_Target->ToCreature()->GetEntry() == NPC_CRYSTALL_SPIKE)
