...
LFGDungeonData::LFGDungeonData() : id(0), name(), map(0), type(0), subtype(0), expansion(0), group(0), contentTuningId(0),
    ...
    for (LfgDungeonSet::const_iterator it = dungeons.begin(); it != dungeons.end(); ++it)
    {
        ...
        uint32 lockStatus = [&]() -> uint32
        {
            ...
            if (dungeon->seasonal && !IsSeasonActive(dungeon->id))
                return LFG_LOCKSTATUS_NOT_IN_SEASON;
            if (dungeon->requiredItemLevel > player->GetAverageItemLevelEquippedAndBag())
