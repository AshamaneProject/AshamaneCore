...
uint32 ScriptMgr::GetDialogStatus(Player* player, Creature* creature)
{
    ...
    GET_SCRIPT_RET(CreatureScript, creature->GetScriptId(), tmpscript, uint32(QuestGiverStatus::ScriptedDefault));
    player->PlayerTalkClass->ClearMenus();
...
uint32 ScriptMgr::GetDialogStatus(Player* player, GameObject* go)
{
    ...
    GET_SCRIPT_RET(GameObjectScript, go->GetScriptId(), tmpscript, uint32(QuestGiverStatus::ScriptedDefault));
    player->PlayerTalkClass->ClearMenus();
...
{
    return uint32(QuestGiverStatus::ScriptedDefault);
}
