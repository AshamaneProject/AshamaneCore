    ...
    switch (e.GetActionType())
    {
        ...
        case SMART_ACTION_SUMMON_CREATURE:
        {
            ...
            for (WorldObject* target : targets)
            {
                ...
                z += e.target.z;
                o += e.target.o;
                if (Creature* summon = summoner->SummonCreature(e.action.summonCreature.creature, x, y, z, o, (TempSummonType)e.action.summonCreature.type, e.action.summonCreature.duration, e.action.summonCreature.isPersonnal, personalSpawn))
                {
                ...
                if (IsCreature(target))
                {
                    switch (e.action.animKit.type)
                    {
                        ...
                        case 3:
                            ...
                            break;
                        case 4:
                            target->ToCreature()->SendPlaySpellVisualKit(e.action.animKit.animKit, 0, 0);
                            break;
                    }
        ...
        case SMART_ACTION_PLAY_SPELL_VISUAL:
        {
            for (WorldObject* const target : targets)
            {
                if (IsUnit(target))
                {
                    ...
                    {
                        case 0:
                            target->ToUnit()->SendPlaySpellVisual(target->GetGUID(), e.action.playSpellVisual.playVisualId, (float)e.action.playSpellVisual.travelSpeed);
                            break;
                        case 1:
                            me->SendPlaySpellVisual(target->GetGUID(), e.action.playSpellVisual.playVisualId, (float)e.action.playSpellVisual.travelSpeed);
                            break;
                        case 2:
                            target->ToUnit()->SendPlaySpellVisual(me->GetGUID(), e.action.playSpellVisual.playVisualId, (float)e.action.playSpellVisual.travelSpeed);
                            break;
                        case 3:
                            target->ToUnit()->SendPlaySpellVisual(target->GetGUID(), e.action.playSpellVisual.playVisualId);
                            break;
                        case 4:
                            me->SendPlaySpellVisual(target->GetGUID(), e.action.playSpellVisual.playVisualId);
                            break;
                        case 5:
                            target->ToUnit()->SendPlaySpellVisual(me->GetGUID(), e.action.playSpellVisual.playVisualId);
                            break;
                    }
                }
            }
        ...
        case SMART_ACTION_PLAY_ORPHAN_SPELL_VISUAL:
        {
            for (WorldObject* const target : targets)
            {
                if (IsUnit(target))
                {
                    ...
                    {
                        case 0:
                            target->ToUnit()->SendPlayOrphanSpellVisual(target->GetGUID(), e.action.playOrphanSpellVisual.playOrphanVisualId, (float)e.action.playOrphanSpellVisual.travelSpeed);
                            break;
                        case 1:
                            me->SendPlayOrphanSpellVisual(target->GetGUID(), e.action.playOrphanSpellVisual.playOrphanVisualId, (float)e.action.playOrphanSpellVisual.travelSpeed);
                            break;
                        case 2:
                            target->ToUnit()->SendPlayOrphanSpellVisual(me->GetGUID(), e.action.playOrphanSpellVisual.playOrphanVisualId, (float)e.action.playOrphanSpellVisual.travelSpeed);
                            break;
                    }
                }
            }
        ...
        case SMART_ACTION_CANCEL_VISUAL:
        {
            for (WorldObject* const target : targets)
            {
                if (IsUnit(target))
                {
                    ...
                    {
                        case 0:
                            target->ToUnit()->SendCancelSpellVisualKit(e.action.cancelSpellVisual.cancelVisualId);
                            break;
                        case 1:
                            target->ToUnit()->SendCancelSpellVisual(e.action.cancelSpellVisual.cancelVisualId);
                            break;
                        case 2:
                            target->ToUnit()->SendCancelOrphanSpellVisual(e.action.cancelSpellVisual.cancelVisualId);
                            break;
                    }
                }
            }
        ...
        case SMART_ACTION_CIRCLE_PATH:
        {
            for (WorldObject* const target : targets)
                if (Unit* unit = target->ToUnit())
                    me->GetMotionMaster()->MoveCirclePath(unit->GetPositionX(), unit->GetPositionY(), unit->GetPositionZ(), (float)e.action.moveCirclePath.radius, e.action.moveCirclePath.clockWise, uint8(e.action.moveCirclePath.stepCount));
        ...
        case SMART_ACTION_CAST_SPELL_OFFSET:
        {
            for (WorldObject* const target : targets)
            {
                ...
                if (e.action.castOffSet.triggered)
                    target->ToUnit()->CastSpell(x, y, z, e.action.castOffSet.spellId, true);
                else
                    target->ToUnit()->CastSpell(x, y, z, e.action.castOffSet.spellId, false);
            }
            break;
        }
        default:
