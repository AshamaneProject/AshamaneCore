...
Creature::Creature(bool isWorldObject): Unit(isWorldObject), MapObject(), m_PlayerDamageReq(0), _pickpocketLootRestore(0),
    ...
    m_wildBattlePet(nullptr), _regenerateHealth(true), _regenerateHealthLock(false)
{
...
void Creature::ReLoad(bool skipDB)
{
    ...
    if (!IsPositionValid())
    {
        TC_LOG_ERROR("entities.unit", "Creature::Create(): given coordinates for creature (SpawnID " UI64FMTD ", entry %d) are not valid (X: %f, Y: %f, Z: %f, O: %f)",
