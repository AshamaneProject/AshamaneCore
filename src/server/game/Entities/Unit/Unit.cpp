...
void Unit::DealDamageMods(Unit const* victim, uint32& damage, uint32* absorb, DamageEffectType damagetype) const
{
    ...
    if (damagetype == DIRECT_DAMAGE)
    {
        ...
        if (difficulty != DIFFICULTY_NONE)
            if (InstanceDifficultyMultiplier const* multiplier = sObjectMgr->GetInstanceDifficultyMultiplier(GetMapId(), difficulty))
    ...
    if (damagetype != NODAMAGE)
    {
       ...
       if (!damage && damagetype != DOT && cleanDamage && cleanDamage->absorbed_damage)
        ...
        for (AuraEffectList::iterator i = vCopyDamageCopy.begin(); i != vCopyDamageCopy.end(); ++i)
        {
            ...
            DealDamageMods(shareDamageTarget, share, nullptr, NODAMAGE);
...
bool Unit::CastSpell(GameObject* go, uint32 spellId, bool triggered, Item* castItem, AuraEffect const* triggeredByAura, ObjectGuid originalCaster)
{
    SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spellId, GetMap()->GetDifficultyID());
    if (!spellInfo)
    {
...
void Unit::DealMeleeDamage(CalcDamageInfo* damageInfo, bool durabilityLoss)
{
    ...
    if (damageInfo->damage)
    {
        ...
        for (AuraEffect const* aurEff : vDamageShieldsCopy)
        {
            ...
            victim->DealDamageMods(this, damage, nullptr, SPELL_DIRECT_DAMAGE);
...
void Unit::CalcAbsorbResist(DamageInfo& damageInfo)
{
    ...
    if (damageInfo.GetVictim() != this)
    {
        ...
        for (AuraEffectList::iterator itr = vSplitDamagePctCopy.begin(); itr != vSplitDamagePctCopy.end() && damageInfo.GetDamage() > 0; ++itr)
        {
            ...
            DealDamageMods(caster, splitDamage, &split_absorb, DIRECT_DAMAGE);
...
SpellMissInfo Unit::MagicSpellHitResult(Unit* victim, SpellInfo const* spellInfo) const
{
    ...
    if (tmp > 0 && rand < tmp && !spellInfo->HasAttribute(SPELL_ATTR8_CANT_MISS))
        return SPELL_MISS_MISS;
...
void Unit::RemoveAurasOnEvade()
{
    ...
    {
        Aura const* aura = aurApp->GetBase();
        return !aura->GetSpellInfo()->HasAura(SPELL_AURA_CONTROL_VEHICLE)
        ...
        if (minion->IsGuardianPet())
        {
            if (Guardian* oldPet = GetGuardianPet())
            {
                if (oldPet != minion && (oldPet->IsPet() || minion->IsPet() || oldPet->GetEntry() != minion->GetEntry()))
                {
                    ...
                    if (oldPet->IsPet())
                        ((Pet*)oldPet)->Remove(PET_SAVE_NOT_IN_SLOT);
                    else
                        oldPet->UnSummon();
...
void Unit::setDeathState(DeathState s)
{
    ...
    if (s == JUST_DIED)
    {
        ModifyAuraState(AURA_STATE_WOUNDED_20_PERCENT, false);
        ModifyAuraState(AURA_STATE_WOUNDED_25_PERCENT, false);
        ModifyAuraState(AURA_STATE_WOUNDED_35_PERCENT, false);
...
void Unit::SetPower(Powers power, int32 val)
{
    ...
    if (IsPlayer())
        sScriptMgr->OnModifyPower(ToPlayer(), power, GetPower(power), val, false, false);
                ...
                if (uint32 journalEncounterId = sObjectMgr->GetCreatureTemplateJournalId(creature->GetCreatureTemplate()->Entry))
                {
                    if (auto items = sDB2Manager.GetJournalItemsByEncounter(journalEncounterId))
                    {
                        ...
                        for (JournalEncounterItemEntry const* item : potentialItems)
                            loot->AddItem(LootStoreItem(item->ItemID, uint8(LootItemType::Item), 0, 10, 0, LOOT_MODE_DEFAULT, 0, 1, 1), looter);
                    }
                }
            }
