...
void Spell::EffectTriggerSpell(SpellEffIndex effIndex)
{
    ...
    if (effectInfo->MiscValue)
    {
        ...
        m_caster->GetScheduler().Schedule(Milliseconds(effectInfo->MiscValue), triggerSpellId, [targetGUID, targetDest, spellInfo, values, originalCasterGUID, triggerSpellId, triggerMask](TaskContext context)
        {
            if (Unit* caster = context.GetUnit())
            {
                ...
                if (!caster->CastSpell(delayedTargets, spellInfo, &values, (TriggerCastFlags)triggerMask, nullptr, nullptr, originalCasterGUID))
    ...
    else
    {
        ...
        m_caster->CastSpell(targets, spellInfo, &values, (TriggerCastFlags)triggerMask, nullptr, nullptr, m_originalCasterGUID);
    }
}
...
void Spell::EffectCreateItem2(SpellEffIndex effIndex)
{
    ...
    if (m_spellInfo->IsLootCrafting())
    {
        if (item_id)
        {
            ...
            player->AutoStoreLoot(m_spellInfo->Id, LootTemplates_Spell, context, false, true);    // create some random items for player spec only
...
void Spell::EffectOpenLock(SpellEffIndex effIndex)
{
    ...
    if (gameObjTarget)
    {
        ...
        else if (goInfo->type == GAMEOBJECT_TYPE_FLAGSTAND || goInfo->type == GAMEOBJECT_TYPE_NEW_FLAG)
        {
            ...
            if (Battleground* bg = player->GetBattleground())
            {
                if (bg->GetTypeID(true) == BATTLEGROUND_EY || bg->GetTypeID(true) == BATTLEGROUND_WS || bg->GetTypeID(true) == BATTLEGROUND_TP)
                    bg->EventPlayerClickedOnFlag(player, gameObjTarget);
                return;
            }
        }
        ...
        else if ((gameObjTarget->GetGOInfo()->type == GAMEOBJECT_TYPE_GOOBER || gameObjTarget->GetGOInfo()->type == GAMEOBJECT_TYPE_NEW_FLAG_DROP) && sOutdoorPvPMgr->HandleOpenGo(player, gameObjTarget))
            return;
    ...
    if (dispelList.empty())
    {
        ...
        for (uint32 i = 0; i < MAX_SPELL_EFFECTS; i++)
            if (SpellEffectInfo const* effect = m_spellInfo->GetEffect(effIndex))
                if (effect->IsEffect(SPELL_EFFECT_DISPEL))
                    lastDispellEffect = effect->EffectIndex;
...
void Spell::EffectLearnPetSpell(SpellEffIndex effIndex)
{
    ...
    SpellInfo const* learn_spellproto = sSpellMgr->GetSpellInfo(m_spellValue->EffectTriggerSpell[effIndex], DIFFICULTY_NONE);
    if (!learn_spellproto)
        return;
...
{
    SpellEffectInfo const* currentEffect = m_spellInfo->GetEffect(effIndex);
...
void Spell::EffectSummonObjectWild(SpellEffIndex effIndex)
{
    ...
    if ((go->GetGoType() == GAMEOBJECT_TYPE_FLAGDROP || go->GetGoType() == GAMEOBJECT_TYPE_NEW_FLAG_DROP) && m_caster->IsPlayer())
        if (Player* player = m_caster->ToPlayer())
            if (Battleground* bg = player->GetBattleground())
...
void Spell::EffectFeedPet(SpellEffIndex effIndex)
{
    ...
    m_caster->CastCustomSpell(pet, m_spellValue->EffectTriggerSpell[effIndex], &benefit, nullptr, nullptr, true);
}
...
void Spell::EffectChargeDest(SpellEffIndex effIndex)
{
    ...
    {
        if (m_spellValue->EffectTriggerSpell[effIndex])
            m_caster->CastSpell(destTarget->GetPositionX(), destTarget->GetPositionY(), destTarget->GetPositionZ(), m_spellValue->EffectTriggerSpell[effIndex], TRIGGERED_NONE, nullptr, nullptr, m_originalCasterGUID);
    }
}
...
void Spell::EffectLeapBack(SpellEffIndex /*effIndex*/)
{
    ...
    if (Player* playerCaster = m_caster->ToPlayer())
        playerCaster->SetFallInformation(0, m_caster->GetPositionZ());
}
...
void Spell::EffectResurrectWithAura(SpellEffIndex effIndex)
{
    ...
    if (sSpellMgr->GetSpellInfo(m_spellValue->EffectTriggerSpell[effIndex], DIFFICULTY_NONE))
...
void Spell::EffectLootWithToast(SpellEffIndex /*effIndex*/)
{
    ...
    ItemContext context = m_spellInfo->HasAttribute(SPELL_ATTR0_TRADESPELL) ? ItemContext::Trade_Skill : ItemContext::NONE;
    unitTarget->ToPlayer()->AutoStoreLoot(m_spellInfo->Id, LootTemplates_Spell, context, false, true, TOAST_METHOD_POPUP);
}
...
void Spell::EffectScrapItem(SpellEffIndex /*effIndex*/)
{
    if (effectHandleMode != SPELL_EFFECT_HANDLE_HIT_TARGET)
        return;
